...
(function () {
  ...
  var ЧАСТОТА_МС = 1500;
  var ИЗ_ДОМАШКИНА = new URLSearchParams(location.search).get("from") === "domashkin";
  ...
  var домашкинСправка = document.getElementById("домашкинСправка");
  if (домашкинСправка) { домашкинСправка.hidden = !ИЗ_ДОМАШКИНА; }
      ...
      .then(function (итог) {
        ...
        э.итог.innerHTML = "<p>Ключи дневника приняты. У нас лежат только они:</p>"
          ...
          + ((итог.resumed_loops || []).length
            ...
            : "")
          + (ИЗ_ДОМАШКИНА
            ? "<p><b>Готово.</b> Данные ребёнка и настройки Домашкина остались на месте — повторно заполнять их не нужно.</p>"
            : "")
  ...
  if (состояние.token) {
    э.весть.textContent = ИЗ_ДОМАШКИНА
      ? "Вы вошли в запись Домашкина. Откройте дневник и завершите повторный вход."
      : "Вы вошли в Фиксар. Можно открывать окно дневника.";
    э.открыть.disabled = false;
  } else {
    э.весть.innerHTML = ИЗ_ДОМАШКИНА
      ? "В этом браузере не найден вход в вашу запись Домашкина. Откройте в его чате команду <b>/setup</b>, войдите по новой кнопке и вернитесь к подключению дневника."
      : "Сначала войдите в Фиксар: <a href=\"/vhod/\">страница входа</a>.";
  }
})();
