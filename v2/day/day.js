...
function participantName(progress) {
  if (!progress.invitee || ["Второй родитель", "Another parent"].includes(progress.invitee)) return tx("Второй родитель", "Another parent");
  return progress.invitee;
}
function assigneeName(progress) {
  if (progress.assigneeId === "helper") return participantName(progress);
  if (progress.assigneeId === "me" || ["Я", "Me"].includes(progress.assignee)) return tx("Я", "Me");
  return progress.assignee || tx("Я", "Me");
}
...
function action(name, agentId, source) {
  ...
  if (name === "space") {
    ...
    openModal(tx("Кто возьмёт следующий шаг?", "Who takes the next step?"), `<form data-form="assign" data-agent="${agentId}"><label class="control">${tx("Ответственный", "Assignee")}<select name="assignee"><option value="me">${tx("Я", "Me")}</option>${caseState(agentId).invited && caseState(agentId).role === "parent" ? `<option value="helper">${esc(participantName(caseState(agentId)))}</option>` : ""}</select></label><label class="control">${tx("Срок в примере", "Sample due date")}<input name="due" type="date" required value="2026-09-18"></label><p class="muted">${tx("Другого участника можно назначить после добавления с ролью «Помощник».", "Add a participant with the Helper role before assigning them.")}</p><div class="dialog-actions"><button class="btn primary" type="submit">${tx("Сохранить назначение", "Save assignment")}</button></div></form>`);
  } else if (name === "notify") {
    ...
    openModal(tx("Всё готово?", "All done?"), `<p>${tx("После подтверждения задача получит статус «Завершено». Материалы останутся доступны.", "Confirm to mark the task as completed. Your materials will remain available.")}</p><div class="notice">${tx("Ответственный", "Assignee")}: ${esc(assigneeName(progress))}<br>${tx("Материал", "Material")}: ${progress.saved ? tx("сохранён", "saved") : tx("ещё не сохранён", "not saved yet")}</div><div class="dialog-actions">${button(tx("Подтвердить завершение", "Confirm completion"), "confirm-complete", "check", "primary", agentId)}${button(tx("Вернуться к делу", "Back to case"), "close", "arrow")}</div>`);
...
document.addEventListener("submit", event => {
  ...
  if (form.dataset.form === "invite") {
    ...
    updateCase(agentId, { assigned: true, assigneeId: helper ? "helper" : "me", assignee: helper ? participantName(progress) : "", due: data.get("due") });
...
document.addEventListener("input", event => {
  ...
  if (field.matches("[data-hue]")) {
    state.hue = Number(field.value); document.documentElement.style.setProperty("--hue", state.hue);
    document.querySelectorAll("[data-hue]").forEach(input => { if (input !== field) input.value = state.hue; });
    document.querySelectorAll("[data-hue-output]").forEach(output => { output.value = `${state.hue}°`; }); persist();
  }
});
